...
def BlendShiftedPredData(model_1, PredData, OriFname):
    print("Create blending image start")
    ...
    dataY = []
    if DENOISE_USING_THREADS:
        print("Denoising using %d threads" %(DenoiseTNum))
        ### Threading method
...
def PredictTestFileAndShow(model_1, flist, findex = 0, file_no = 999):
    ...
    if DENOISE_USING_THREADS:       ### not recommanded
        print("Denoising using %d threads"  %(DenoiseTNum))
        ### Threading method
        for j in range(int(Y_Len / DenoiseTNum)): ### create 16 threads
            thread = []
        ...
        ### serial method - significanlly faster than threads
        for j in range(Y_Len):
            dataX = []
            for i in range(X_Len):
